...
export default function AgentDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Get agent info from managed list
      const managed = await getAdminManagedAgents(user.id);
      ...
      // Load sessions and compute scores
              ...
              <div className="rounded-2xl overflow-hidden" style={{ background: '#0d0d0d', border: '1px solid rgba(215,255,0,0.1)' }}>
                ...
                {sessions.map(({ session, scores }) => (
                  <SessionRow
                    ...
                    session={session}
                    scores={scores}
                    onClick={() => router.push(`/results/${session.id}?from=admin`)}
                  />
                ))}
              </div>
            )}
