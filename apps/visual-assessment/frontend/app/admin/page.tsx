...
import { getAdminManagedAgents, getAgentSessions } from '@/lib/supabase-helpers';
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="flex gap-3 mt-2">
            <button
              ...
              style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.1)', fontFamily: 'var(--font-space)' }}
            >
              Cancel
            </button>
            <button
      ...
      {sessionCount === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {goodSections.length > 0 && (
            <div>
              <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(215,255,0,0.5)', fontFamily: 'var(--font-space)' }}>
                Strong
              </div>
          ...
          {weakSections.length > 0 && (
            <div>
              <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(239,68,68,0.6)', fontFamily: 'var(--font-space)' }}>
                Needs Work
              </div>
              <div className="flex flex-wrap gap-1.5">
...
export default function AdminPage() {
  ...
  const loadAgents = useCallback(async (aId: string) => {
    setLoading(true);
    try {
      const managed = await getAdminManagedAgents(aId);
    ...
    <main className="min-h-screen" style={{ background: 'var(--tgl-black)' }}>
      ...
      <div className="px-6 py-8 max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-6 mb-8">
          ...
          {agents.filter(a => a.sessionCount > 0).length > 0 && (
            <>
              <div style={{ width: 1, height: 32, background: 'rgba(255,255,255,0.08)' }} />
              <div>
                <div className="text-3xl font-black" style={{ color: 'var(--tgl-lime)', fontFamily: 'var(--font-space)', letterSpacing: '-0.04em' }}>
                  {Math.round(
                    agents.filter(a => a.sessionCount > 0).reduce((s, a) => s + a.overall, 0) /
                    agents.filter(a => a.sessionCount > 0).length * 100
                  )}%
                </div>
                <div className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.4)', fontFamily: 'var(--font-montserrat)' }}>
                  Team average
        ...
        {loading ? (
          <div className="flex items-center justify-center py-24">
            <div className="text-sm" style={{ color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--font-montserrat)' }}>
              Loading agents…
            </div>
          </div>
        ) : agents.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {agents.map(data => (
              <AgentCard
                key={data.agent.id}
                data={data}
                onClick={() => router.push(`/admin/agent/${data.agent.agent_user_id}`)}
              />
            ))}
          </div>
        )}
