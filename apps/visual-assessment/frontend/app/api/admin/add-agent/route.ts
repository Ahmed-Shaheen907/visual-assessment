...
export async function POST(req: NextRequest) {
  ...
  // Standalone client (no cookies) — won't affect admin session
  ...
  // Upsert into va_admin_managed_agents
  const { error: upsertError } = await supabaseServer
    .from('va_admin_managed_agents')
    .upsert(
      { admin_user_id: adminUser.id, agent_user_id: agentUserId, agent_email: agentEmail, agent_name: agentName },
      { onConflict: 'admin_user_id,agent_user_id' }
    );
