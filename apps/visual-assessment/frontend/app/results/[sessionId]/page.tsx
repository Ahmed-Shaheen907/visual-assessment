      ...
      <div className="flex-1 px-6 py-8 max-w-3xl mx-auto w-full">
        ...
        {answerGroups.length > 0 && (
          <div className="rounded-2xl overflow-hidden mb-8" style={{ border: '1px solid rgba(215,255,0,0.1)' }}>
            ...
            {answerGroups.map((group, gi) => (
              <div key={group.key}>
                {/* Group label */}
                <div className="px-5 py-2.5" style={{ background: 'rgba(255,255,255,0.025)', borderTop: gi > 0 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
                  <span className="text-xs font-bold uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.35)', fontFamily: 'var(--font-space)' }}>
                    {group.label}
                  </span>
                </div>
                {/* Answer rows */}
                {group.rows.map((row, ri) => (
                  <div
                    ...
                    style={{ borderTop: '1px solid rgba(255,255,255,0.04)', background: row.correct ? 'transparent' : 'rgba(239,68,68,0.025)' }}
                  >
                    <div
                      className="shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold mt-0.5"
                      style={{ background: row.correct ? 'rgba(215,255,0,0.15)' : 'rgba(239,68,68,0.15)', color: row.correct ? 'var(--tgl-lime)' : '#ef4444', fontFamily: 'var(--font-space)' }}
                    >
                      {row.correct ? '✓' : '✗'}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs mt-1" style={{ fontFamily: 'var(--font-montserrat)', color: row.correct ? 'rgba(215,255,0,0.6)' : 'rgba(255,255,255,0.4)' }}>
                        You answered: <span className="font-semibold" style={{ color: row.correct ? 'var(--tgl-lime)' : '#ef4444' }}>{row.given ?? '—'}</span>
                      </div>
                      {!row.correct && row.correctAnswer && (
